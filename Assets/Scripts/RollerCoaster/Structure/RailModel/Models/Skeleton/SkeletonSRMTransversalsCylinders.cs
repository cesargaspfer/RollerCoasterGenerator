...
public class SkeletonSRMTransversalsCylinders : SubRailModel
{
    ...
    public override RailSurface GetRailSurface(RailType type)
    {
        ...
        float mainCylinderRadius = SkeletonRMProperties.mainCylinderRadius * 0.75f;
        float sideCylinderRadius = SkeletonRMProperties.sideCylinderRadius * 0.75f;
    ...
    public override Mesh GenerateMesh(RailType type, ExtrudedRailSurface esr)
    {
        ...
        for (int i = 0; i < vertices.Length - 1; i++)
        {
            for (int j = 0; j < (vertices[i].Length - 3) / 3; j++)
            {
                ...
                int y1 = 1 * _modelResolution + j;
                int y2 = 2 * _modelResolution + j;
                triangles[index] = CalculateIndex(i, y1, vertices[i].Length);
                triangles[index + 1] = CalculateIndex(i + 1, y2, vertices[i].Length);
                triangles[index + 2] = CalculateIndex(i, y1 + 1, vertices[i].Length);

                triangles[index + 3] = CalculateIndex(i, y1 + 1, vertices[i].Length);
                triangles[index + 4] = CalculateIndex(i + 1, y2, vertices[i].Length);
                triangles[index + 5] = CalculateIndex(i + 1, y2 + 1, vertices[i].Length);
                
                k = 1;
                index = 6 * (i * vertices[i].Length + k * _modelResolution + j);
