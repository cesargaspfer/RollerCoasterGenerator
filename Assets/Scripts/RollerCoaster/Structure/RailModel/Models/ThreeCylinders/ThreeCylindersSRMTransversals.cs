...
public class ThreeCylindersSRMTransversals : SubRailModel
{
    private float _zOffset = 0.025f;
    ...
    public override RailSurface GetRailSurface(RailType type)
    {
        ...
        _zOffset = ThreeCylindersRMProperties.zOffset;
        
        Vector3[] points = new Vector3[8];
        Vector3[] normals = new Vector3[8];

        // Generating points and normals
        points[0] = new Vector3(_zOffset, 0f, 0f);
        points[1] = new Vector3(_zOffset, 0.3f, -0.3f);
        points[2] = new Vector3(_zOffset, 0.2f, 0f);
        points[3] = new Vector3(_zOffset, 0.3f, 0.3f);
        for (int i = 0; i < 4; i++)
        {
